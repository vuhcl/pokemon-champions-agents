...
| Path | Role |
|------|------|
| `data/legality/champions.v1.json` | Full effective legality surface for mod `champions` (current regulation) |
| `data/legality/fixtures/{prior}_to_champions.diff.json` | **Live** prior→current flips (e.g. `championsregmb`→`champions` while Reg M-C is current). Regenerated by extract; `from_mod` is derived from the champions VGC format letter |
| `data/legality/fixtures/championsregma_to_champions.diff.json` | Historical M-A→M-B flips — static answer key; not regenerated |
| `data/legality/fixtures/championsregmb_to_champions.diff.json` | After the next regulation lands, this file becomes historical (abandoned live path); while M-C is current it is also the live fixture |
...
| `data/legality/fixtures/regulation_news_bookmark.json` | Last champions-news page id scanned for regulation Duration (not a calendar date) |
...
Live fixture: `{from_mod}_to_champions.diff.json` where `from_mod` is the archived
prior Showdown mod (`championsregm{prevLetter}` derived from the current champions
VGC `Reg M-*` letter). Historical fixtures (e.g. `championsregma_…`, and later an
abandoned `championsregmb_…`) are not overwritten by extract.
...
```ts
{
  meta: {
    ...
    from_mod: string,              // e.g. "championsregmb"
    to_mod: "champions",
  },
  species: [{
    id: string,
...
Items side: compare effective maps `base⊕champions⊕prior` vs `base⊕champions`.
