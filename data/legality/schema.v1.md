...
| `data/legality/fixtures/championsregmb_to_champions.diff.json` | Live prior→current flips (`championsregmb` / Reg M-B → `champions` / Reg M-C); regenerated by extract |
| `data/legality/fixtures/championsregma_to_champions.diff.json` | Historical M-A→M-B flips only — kept as static answer key; not regenerated (`championsregma` gone upstream) |
...
Live fixture: `championsregmb_to_champions.diff.json` (`from_mod: "championsregmb"`).
Historical M-A fixture keeps `from_mod: "championsregma"` and is not regenerated.
...
Items side: compare effective maps `base⊕champions⊕regmb` vs `base⊕champions`.
