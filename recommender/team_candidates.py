...
def candidate_core_slot_conflicts(
    ...
    *,
    ...
    Same scarce-resource question as candidate_wastes_core_slot, but
    returns identity (slot/species/resource) so masked-core discovery
    knows what to mask. Empty when there is no conflict.
    """
    if not is_core_slot:
        return ()
...
def _preferred_category(preference: TeamCompletionPreference | None) -> str:
    return "B" if preference == "support" else "A"
...
def select_diverse_candidates(
    ...
    *,
    ...
    real, multi-dimensional value; otherwise falls back to the preferred
    category's top pick (A, or B when preference is "support"), then the
    remaining of A/B/C. Alternatives: the top pick from each of the OTHER
    ...
    # (default falls through preferred then remaining A/B/C; alternatives
    # skip empty categories naturally) already handles an empty category
    # correctly.
    ranked_b = [
        c
    ...
    preferred = _preferred_category(preference)
    category_order = (preferred, *(k for k in ("A", "B", "C") if k != preferred))
    ...
    if genuine_multi_signal_lineages:
        ...
        # best in the preferred category (A unless preference is
        # "support"), falling back to its rank in whichever category
        # it's strongest in otherwise.
        best_rank = None
            ...
            for key, ranked in ranked_by_category.items():
                for i, c in enumerate(ranked):
                    if frozenset(lineage_ids(c.species)) == lineage:
                        candidate_rank = (i, key != preferred)
    ...
    for key in (*(k for k in ("A", "B", "C") if k != preferred), preferred):
        ranked = ranked_by_category[key]
...
def independently_strong_category_a(
    ...
    """Category A top-3 when wastes_core_slot is ignored (current field)."""
...
def _search_gap_fill(
    ...
    if shared is not None and shared.status == "available":
        shared_ids = {to_id(row.species_id) for row in (shared.rows or ())}
