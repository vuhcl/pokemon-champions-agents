...
## 2026-09-01 — Session summary: role-aware synthesis, masked-core
clarity, full locked-slot re-pick, and claim_correction's remaining
scope closed

Four units shipped, closing out essentially the entire backlog
generated by the original live-transcript investigation that began with
the Heliolisk hallucination: role-aware build synthesis (ADR-053, the
data-source half already closed by ADR-052), masked-core resolution
label clarity and a dedup safeguard (ADR-054), full species re-pick on
a locked slot with real composition-gap flagging (ADR-055, closing
Shape A after Shape B shipped under ADR-049), and claim_correction's
extension to ability and item claims (ADR-051 Amendment 2026-09-01a,
closing the one explicitly-deferred gap from that pathway's original
v1 ship).

**A pattern worth naming, since it recurred twice more in this
stretch:** both ADR-053 and ADR-055 caught real regressions specifically
because a broader, pre-existing test (a full parametrized round-trip
across all 25 target roles; a checkpoint-serialization round-trip
unrelated to the new feature's own test file) was run in addition to
the narrower validation commands a plan explicitly named. Neither
regression was hypothetical or edge-case — one was a genuinely
pre-existing, checked-in bad cache entry that had simply never been
queried with the right moveset before; the other was a type
inconsistency invisible until data actually passed through the real
serialization boundary. Running the full suite, not just a plan's named
commands, keeps finding real things.

**With this session's queue effectively closed** (team review display
verbosity is the one remaining item, not yet started), essentially
every finding from the original live-transcript investigation —
compendium tier-sort, tier-1 item-clause awareness, redundant-provider
generalization, the full CLI idle-turn chain, real constraint
enforcement, the claim-correction pathway, in-game build preference,
role-aware synthesis, masked-core clarity, and both locked-slot revision
shapes — has been discovered, decided, implemented, and independently
re-verified against real source and real execution, not taken on
report alone.
