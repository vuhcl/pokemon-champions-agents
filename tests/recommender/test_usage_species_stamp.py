...
def test_ingame_flavor_names_stamp_calc_valid_ids():
    for sid, flavor in _REMAP:
        built = set_from_ingame(sid, regulation="champions-reg-mb")
...
def test_featured_or_common_set_stamps_calc_valid_ids():
    for sid, flavor in _REMAP:
        built = featured_or_common_set(sid, regulation="champions-reg-mb")
        assert built is not None, sid
        assert to_id(built["species"]) == sid
        assert built["species"] != flavor
...
def test_basculegion_and_alolan_ninetales_keep_showdown_hyphenation():
    assert featured_or_common_set("basculegion", regulation="champions-reg-mb")["species"] == "Basculegion"
    assert (
        featured_or_common_set("ninetalesalola", regulation="champions-reg-mb")["species"]
        == "Ninetales-Alola"
    )


def test_floette_excluded_from_ingame_slice():
    assert set_from_ingame("floette", regulation="champions-reg-mb") is None


def test_relevant_threats_glue_maushold_vivillon():
    threats = get_relevant_threats({"regulation_mod": "champions-reg-mb"}, n=120)  # type: ignore[arg-type]
