...
def _abort_if_unguarded() -> None:
    ...
    if "rewrite_pending_response_message" not in inspect.getsource(ti._payload_for):
        ...
        sys.exit(2)
    if not hasattr(sc, "iter_verifiable_claims_from_message"):
        print(
            "ABORT: iter_verifiable_claims_from_message missing — need #196 tree.",
            file=sys.stderr,
        )
        sys.exit(2)
...
def main() -> int:
    ...
    for pending_kind, ctx, phrases in scen.GAP_FILL_PROBES:
        ...
        for phrase in phrases:
            attempts[site] += 1
            probe_i += 1
            bind_correlation(turn=probe_base + probe_i, thread_id=thread_id)
